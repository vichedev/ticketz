  ...
  try {
    ...
    if (tokenData && ticket.status !== "pending") {
      if (tokenData.profile !== "admin" && ticket.userId !== parseInt(tokenData.id)) {
        throw new AppError("Sólo el usuario activo del ticket o el Admin pueden realizar cambios en el ticket");
      }
    }
    ...
    if (status !== undefined && ["closed"].indexOf(status) > -1) {
      ...
      if (setting?.value === "enabled") {
        if (ticketTraking.ratingAt == null && !justClose) {
          ...
          bodyRatingMessage +=
            "Digite de 1 a 3 para calificar el atendimiento:\n*1* - _Insatisfecho_\n*2* - _Satisfecho_\n*3* - _Muy Satisfecho_\n\n";
    ...
    if (oldQueueId !== queueId && !isNil(oldQueueId) && !isNil(queueId)) {
      ...
      if (ticket.channel === "whatsapp") {
        ...
        if(transferMessage){
          ...
          const queueChangedMessage = await wbot.sendMessage(
            ...
            {
              text: "\u200eHa sido transferido, pronto iniciaremos su servicio."
            }
          );
          await verifyMessage(queueChangedMessage, ticket, ticket.contact);
        }
      ...
      if (["facebook", "instagram"].includes(ticket.channel)) {
        console.log(`Checking if ${ticket.contact.number} is a valid ${ticket.channel} contact`)
        await sendFaceMessage({ body: "\u200eHa sido transferido, pronto iniciaremos su servicio.", ticket });
      }
    }
